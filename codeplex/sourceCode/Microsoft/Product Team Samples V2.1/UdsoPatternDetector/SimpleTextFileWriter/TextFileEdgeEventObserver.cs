...
namespace StreamInsight.Samples.Utility
{
    ...
    /// <summary>
    /// Edge version of the simple text file writer output.
    /// <para/>
    /// TextFileEdgeOutput generates edge events into a CSV file with the format:
    /// 
    ...
    /// <para/>
    /// </summary>
   public class TextFileEdgeEventObserver<TPayload> : TextFileObserverBase<TPayload, EdgeEvent<TPayload>>
   {
       public TextFileEdgeEventObserver(string fileName, string delimiter)
           : base(fileName, delimiter)
       { 
       }

       protected override void OnNext(EdgeEvent<TPayload> currentEvent, StreamWriter writer)
       {
            ...
            else
            {
                writer.Write("INSERT");
                writer.Write(Delimiter);
                writer.Write(currentEvent.EdgeType);
                writer.Write(Delimiter);
                writer.Write(GetTimeByCulture(currentEvent.StartTime.ToString()));
                writer.Write(Delimiter);
