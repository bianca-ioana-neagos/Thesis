...
{
    /// <summary>
    /// Simulated input adapter for point events.  Will periodically raise 
    /// simulated event data.  If a class implementing the 
    /// ITypeInitializer<TPayload> interface is specified in the configuration
    /// the events will be filled in by an instance of that class.
    /// </summary>
    /// <typeparam name="TPayload"></typeparam>
    public class SimulatedInputPointAdapter<TPayload> : 
        TypedPointInputAdapter<TPayload> 
    {
        /// <summary>
        ...
        public SimulatedInputPointAdapter(SimulatedInputAdapterConfig config)
        {
        ...
        private void RaiseEvent(object state)
        {
            ...
            lock (lockObj)
            {
                PointEvent<TPayload> currEvent = CreateInsertEvent();
                if (currEvent == null)
                    return;
