...
{
    public class SimulatedInputEdgeAdapter<TPayload> :
        TypedEdgeInputAdapter<TPayload>
    {
        /// <summary>
        ...
        private EdgeEvent<TPayload> lastEvent = null;
        ...
        private void RaiseEvent(object state)
        {
            ...
            lock (lockObj)
            {
                ...
                if (lastEvent != null)
                {
                    EdgeEvent<TPayload> closeEvent = CreateInsertEvent(EdgeType.End);
                    closeEvent.StartTime = lastEvent.StartTime;
                    closeEvent.EndTime = DateTimeOffset.Now;
                    closeEvent.Payload = lastEvent.Payload;

                    // If the event cannot be enqueued, release the memory and signal that
                    // the adapter is ready to process more events (via. Ready())
                    if (EnqueueOperationResult.Full == Enqueue(ref closeEvent))
                    {
                        ReleaseEvent(ref closeEvent);
                        Ready();
                    }
                ...
                EdgeEvent<TPayload> currEvent = CreateInsertEvent(EdgeType.Start);
                if (currEvent == null)
                    return;
